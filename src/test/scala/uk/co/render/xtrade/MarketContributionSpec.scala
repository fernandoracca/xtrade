...
import model.{ContributionFailure, ContributionSuccess}
import org.specs2._
...
class MarketContributionSpec  extends SpecificationWithJUnit { def is =
	...
	def e1 = Main().contribute(123456) must be equalTo(ContributionSuccess)
	def e2 = Main().contribute(654321) must be equalTo(ContributionFailure)
